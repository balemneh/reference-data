...
class ChangeRequestControllerIntegrationTest {
    ...
    void testExecuteChangeRequest() throws Exception {
        ...
        assertThat(countryRepository.findByCountryCodeAndCodeSystemCode("EX", "TEST-API")).isPresent();
    }
    ...
    void testInvalidWorkflowTransitions() throws Exception {
        ...
        mockMvc.perform(put("/v1/change-requests/{id}/approve", savedRequest.getId())
                ...
                .andExpect(jsonPath("$.detail").containsString("not in PENDING status"));
    }

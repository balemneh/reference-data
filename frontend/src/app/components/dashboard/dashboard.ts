...
export class DashboardComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  ngOnInit() {
    ...
    this.featureFlagsService.flags$
      ...
      .subscribe(flags => {
        ...
        // Reload dashboard data when flags change
  ...
  loadDashboardData() {
    ...
    return this.apiService.getDashboardStats().subscribe({
      next: (dashboardData) => {
        ...
        // Also update animated stats directly to show immediately
        this.animatedStats.countries.current = dashboardData.countries?.total || 0;
        this.animatedStats.ports.current = dashboardData.ports?.total || 0;
        this.animatedStats.airports.current = dashboardData.airports?.total || 0;
        this.animatedStats.mappings.current = dashboardData.mappings?.total || 0;
        this.animatedStats.pendingRequests.current = dashboardData.pendingRequests || 0;
  ...
  isCardEnabled(card: string): boolean {
    if (!this.featureFlags) return true; // Default to showing if flags not loaded

    // Safely access nested properties with optional chaining
    const dashboard = this.featureFlags.dashboard;
    const referenceData = this.featureFlags.referenceData;
    const features = this.featureFlags.features;

    switch(card) {
      case 'countries':
        // Only check referenceData flag - if countries data is disabled, don't show on dashboard
        return referenceData?.countries !== false;
      case 'ports':
        // Only check referenceData flag - if ports data is disabled, don't show on dashboard
        return referenceData?.ports !== false;
      case 'airports':
        // Only check referenceData flag - if airports data is disabled, don't show on dashboard
        return referenceData?.airports !== false;
      case 'carriers':
        // Only check referenceData flag - if carriers data is disabled, don't show on dashboard
        return referenceData?.carriers !== false;
      case 'changeRequests':
        // Check features flag for change requests
        return features?.changeRequests !== false;
      case 'analytics':
  ...
  isFeatureEnabled(feature: string): boolean {
    ...
    return (this.featureFlags.features as any)?.[feature] ?? true;
  }

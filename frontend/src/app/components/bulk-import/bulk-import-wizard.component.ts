...
import { ImportExportHistory } from '../import-export/import-export';
...
export class BulkImportWizardComponent implements OnInit, OnDestroy {
  ...
  // Processing
      ...
      this.uploadFile()
        ...
        .subscribe({
          next: (response) => {
            ...
            const operationId = this.addToHistory('PROCESSING');
    ...
    private uploadFile() {
      ...
      return this.apiService.initiateBulkImport(formData).pipe(
    ...
    updateErrorFilter(): void {
      ...
      if (this.errorFilter === 'ALL') {
        this.filteredErrors = [...errors, ...warnings];
      } else {
        this.filteredErrors = [
          ...errors.filter(e => e.severity === this.errorFilter),
          ...warnings.filter(w => w.severity === this.errorFilter)
        ];
      }
    }
    ...
    private generateValidationReportCSV(): string {
      ...
      const rows = errors.map(error => {
        return [
          ...
          error.column,
          error.field,
          `"${String(error.value).replace(/"/g, '""')}"`, // Corrected escaping for double quotes within strings
          error.severity,
          `"${error.message.replace(/"/g, '""')}"`, // Corrected escaping for double quotes within strings
          `"${(error.suggestion || '').replace(/"/g, '""')}"` // Corrected escaping for double quotes within strings
        ].join(',');
      }).join('\n');
  ...
  private addToHistory(status: 'COMPLETED' | 'FAILED' | 'PARTIAL' | 'PROCESSING', processedCount = 0, failedCount = 0): string {
    if (!this.selectedFile) return '';

    const historyEntry: ImportExportHistory = {
      id: `op_${Date.now()}`,
      operationType: 'IMPORT',
      entityType: this.dataType,
      fileName: this.selectedFile.file.name,
      fileSize: this.selectedFile.size,
      format: this.selectedFile.format,
      ...
      changeRequestId: this.changeRequestId || undefined,
      batchId: this.batchId || undefined
    };

    this.operationHistoryService.addOperation(historyEntry);
